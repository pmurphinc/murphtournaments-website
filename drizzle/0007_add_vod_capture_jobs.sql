CREATE TABLE IF NOT EXISTS `vod_capture_jobs` (
	...
	CONSTRAINT `vod_capture_jobs_vodAnalysisId_vod_analyses_id_fk` FOREIGN KEY (`vodAnalysisId`) REFERENCES `vod_analyses`(`id`) ON DELETE cascade ON UPDATE no action,
	INDEX `vod_capture_jobs_vodAnalysisId_idx` (`vodAnalysisId`),
	INDEX `vod_capture_jobs_status_idx` (`status`),
	INDEX `vod_capture_jobs_createdAt_idx` (`createdAt`)
);
