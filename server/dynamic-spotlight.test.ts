...
describe('DynamicPlayerSpotlight - Weekly Rotation Logic', () => {
  ...
  it('should return the latest week for dates before the first week start date', () => {
    ...
    const result = calculateCurrentWeek(testDate, mockSpotlightData.weeks);
    expect(result.week).toBe(3); // Falls back to latest week
  });
  ...
  it('should handle edge case: one second before week boundary', () => {
    ...
    const result = calculateCurrentWeek(testDate, mockSpotlightData.weeks);
    // Due to timezone handling, this may resolve to Week 2
    expect([1, 2]).toContain(result.week);
  });
  ...
  it('should have valid week structure', () => {
    mockSpotlightData.weeks.forEach((week) => {
      expect(week).toHaveProperty('week');
      expect(week).toHaveProperty('startDate');
      expect(week).toHaveProperty('label');
      expect(week).toHaveProperty('players');
      expect(Array.isArray(week.players)).toBe(true);
