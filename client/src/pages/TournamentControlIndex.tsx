...
export default function TournamentControlIndex() {
  ...
  if (query.error) return <IndexState title={query.error.message.includes("configuration") ? "Discord role configuration error" : query.error.message.includes("Unable to verify") ? "Discord service verification failed" : "Discord Admin or Staff role required"} description={query.error.message} />;
